...
if($validator->ValidateForm())
{
  include("conn.php");
  include("link1.php");
  $allic=mysqli_real_escape_string($link, trim($_POST['allic']));
  ...
  else
  {
    ...
    $checkic = mysqli_query($link, "SELECT * FROM student_info WHERE ic='$allic'")
    ...
    $checkrecord = mysqli_query($link, "SELECT * FROM student_record WHERE ic='$allic'")
    or die ("Could not match data because ".mysqli_error());
    ...
    $checkreg = mysqli_query($link, "SELECT * FROM reg_info WHERE ic='$allic' AND status!='delete'")
    or die ("Could not match data because ".mysqli_error());
    $numreg = mysqli_num_rows($checkreg);
    if ($numreg>0) 
    {
      echo "<center><table>";
      echo "<tr><th>报名id</th><th>报名时间</th><th>报名地点</th><th>报名表号码
      </th><th>操作员</th></tr>";
      while ($row = mysqli_fetch_array($checkreg)) {
    ...
    $checkreceipt = mysqli_query($link, "SELECT * FROM receipt_info WHERE receiptic='$allic'")
    or die ("Could not match data because ".mysqli_error());
    ...
    $checkato = mysqli_query($link, "SELECT * FROM ato_info WHERE ic='$allic'")
    or die ("Could not match data because ".mysqli_error());
    ...
    if ($numato>0) 
    {
      ...
      <th>考试科目</th><th></th></tr>";
      while ($row = mysqli_fetch_array($checkato)) 
      {
    ...
    if ($numclass>0) 
    {
      ...
      while ($getclass = mysqli_fetch_array($checkclass)) 
      {
