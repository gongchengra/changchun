...
$ato_date=strtotime($_POST['ato_startdate']);
$ato_date1=strtotime($_POST['ato_enddate'])+24*3600;
$branch=$_POST['hiddenbranch'];
$i=1;
for($tmpdate=$ato_date; $tmpdate<=$ato_date1; $tmpdate=$tmpdate+24*3600)
{
	$tmpdate1=$tmpdate+24*3600;
	$i=$i+1; 
	$objActSheet->setCellValue('A'.$i,date('Y-m-d',$tmpdate));
	$atoresult=mysql_query("SELECT * FROM ((SELECT * FROM ato_info 
	WHERE UNIX_TIMESTAMP(examtime)>'$tmpdate' 
	AND UNIX_TIMESTAMP(examtime)<='$tmpdate1'
	...
	$place='changchun';
	$time='0';
	$examtime9=$tmpdate+9*3600;
	$examtime14=$tmpdate+14*3600;
	$examtime19=$tmpdate+19*3600;
